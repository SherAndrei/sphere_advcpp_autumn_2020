...
void Socket::set_sndtimeo(ssize_t sec, ssize_t usec) const {
    ...
    int error = setsockopt(_id, SOL_SOCKET, SO_SNDTIMEO,
...
void Socket::set_rcvtimeo(ssize_t sec, ssize_t usec) const {
    timeval timeout = { sec, usec };
    int error = setsockopt(_id, SOL_SOCKET, SO_RCVTIMEO,
                   &timeout, sizeof(timeout));
    if (error == -1) {
...
void Socket::set_reuseaddr() const {
    int opt = 1;
    if (setsockopt(_id, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(int)) == -1) {
...
void Socket::set_nonblock() const {
    int flags;
    if ((flags = fcntl(_id, F_GETFL)) == -1) {
        throw SocketOptionError(std::strerror(errno), "O_NONBLOCK");
    }
    if ((fcntl(_id, F_SETFL, flags | O_NONBLOCK)) == -1) {
        throw SocketOptionError(std::strerror(errno), "O_NONBLOCK");
    }
}
