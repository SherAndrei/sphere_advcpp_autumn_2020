...
bool HttpService::try_reset_last_activity_time(net::IClientPlace* p_place) {
    ITimed* p_timed = dynamic_cast<ITimed*>(get(p_place->p_client->conn.get()));
    std::scoped_lock lock(timeout_mutex_, p_timed->mutex());
    if (p_place->p_client->conn->socket().valid()) {
